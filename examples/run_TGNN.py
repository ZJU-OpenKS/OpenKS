...
from openks.models.pytorch.TGNN.arguments import arg_parse
from openks.models.pytorch.TGNN.model import train_student, GNN, RW_NN
from openks.models.pytorch.TGNN.aug import TUDataset_aug
from openks.models.pytorch.TGNN.util import nomalize_data
from openks.models.pytorch.TGNN.compress_loss import CompReSSMomentum


device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

def train(args, teacher, student, compress):
    teacher.train()
    student.train()
    ...
    for data, data_un in zip(train_loader, unsup_train_loader):
        ...
        sup_loss = sup_loss_t # + sup_loss_s

        # unsupervised loss
        unsup_loss = train_student(data_un, teacher, student, compress)
...
if __name__ == '__main__':
    ...
    result_file = open("./result_file_tgnn.txt", mode="a", encoding="utf-8")
    result_file.write("%s"%args.DS + "\t" + str(test_acc_t) + "\t" + str(test_acc_s) + "\n")
    result_file.close()
