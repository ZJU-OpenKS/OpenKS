...
class multi_HAN(nn.Module):
    def __init__(self, n_nodes_list, neighs_type, args):
        ...
        if self.dataset == 'yelp':
            ...
            # self.business_facet_init = nn.ModuleList()
            # self.city_facet_init = nn.ModuleList()
            ...
            #     self.business_facet_init.append(nn.Embedding(n_businesses+1, len(self.neighs_type[1]) * self.n_facet[l], padding_idx=n_businesses))
            #     self.city_facet_init.append(nn.Embedding(n_cities+1, len(self.neighs_type[2]) * self.n_facet[l], padding_idx=n_cities))
            ...
            #     torch.nn.init.ones_(self.business_facet_init[l].weight)
            #     torch.nn.init.ones_(self.city_facet_init[l].weight)
            #     torch.nn.init.ones_(self.category_facet_init[l].weight)
            #     self.user_facet_init[l].weight.data[n_users] = 0
            #     self.business_facet_init[l].weight.data[n_businesses] = 0
            #     self.city_facet_init[l].weight.data[n_cities] = 0
            #     self.category_facet_init[l].weight.data[n_categories] = 0
            # self.facet_init = nn.ModuleList([self.user_facet_init, self.business_facet_init, self.city_facet_init, self.category_facet_init])

        elif self.dataset == 'amazon':
            # self.user_emb = nn.ModuleList()
            # self.item_emb = nn.ModuleList()
            # self.brand_emb = nn.ModuleList()
            # self.category_emb = nn.ModuleList()

            # n_users, n_items, n_brands, n_categories = self.n_nodes_list
            # for i in range(self.n_layer):
            #     user_emb_init = nn.Embedding(n_users + 1, self.cur_dim, padding_idx=n_users)
            #     item_emb_init = nn.Embedding(n_items + 1, self.cur_dim, padding_idx=n_items)
            #     brand_emb_init = nn.Embedding(n_brands + 1, self.cur_dim, padding_idx=n_brands)
            #     category_emb_init = nn.Embedding(n_categories + 1, self.cur_dim, padding_idx=n_categories)
            #     stdv = 1. / np.sqrt(self.cur_dim)
            #     torch.nn.init.uniform_(user_emb_init.weight, -stdv, stdv)
            #     torch.nn.init.uniform_(item_emb_init.weight, -stdv, stdv)
            #     torch.nn.init.uniform_(brand_emb_init.weight, -stdv, stdv)
            #     torch.nn.init.uniform_(category_emb_init.weight, -stdv, stdv)
            #     user_emb_init.weight.data[n_users] = 0
            #     item_emb_init.weight.data[n_items] = 0
            #     brand_emb_init.weight.data[n_brands] = 0
            #     category_emb_init.weight.data[n_categories] = 0
            #     self.user_emb.append(user_emb_init)
            #     self.item_emb.append(item_emb_init)
            #     self.brand_emb.append(brand_emb_init)
            #     self.category_emb.append(category_emb_init)
            # self.emb = nn.ModuleList([self.user_emb, self.item_emb, self.brand_emb, self.category_emb])
            ...
            # torch.nn.init.xavier_uniform_(self.user_emb_init.weight)
            # torch.nn.init.xavier_uniform_(self.movie_emb_init.weight)
            # torch.nn.init.xavier_uniform_(self.actor_emb_init.weight)
            # torch.nn.init.xavier_uniform_(self.director_emb_init.weight)
            # torch.nn.init.xavier_uniform_(self.country_emb_init.weight)
            # torch.nn.init.xavier_uniform_(self.genre_emb_init.weight)
        ...
        # self.homo_encoders = nn.ModuleList([HomoAggregate_attention(self.cur_dim, self.n_facet[l], self.dropout, self.training)
        #                                                                 for l in range(self.n_layer)])

        # self.hete_encoders = nn.ModuleList([HeteAttention(self.cur_dim, self.n_facet[0], len(self.neighs_type[i]))
        #                                                                     for i in range(len(self.n_nodes_list))])

        self.hete_encoders = nn.ModuleList([nn.ModuleList([HeteAttention(self.cur_dim, self.n_facet[l], len(self.neighs_type[i]))
                                                                            for i in range(len(self.n_nodes_list))])
                                                                            for l in range(self.n_layer)])
        ...
        # self.homo_encoders = nn.ModuleList([nn.ModuleList([HomoAggregate_attention(self.cur_dim, self.n_facet[0], self.dropout, self.training)
        #                                                 for i in range(len(self.neighs_type[type]))])
        #                                                 for type in range(len(self.n_nodes_list))])

        # self.pre_encoders = nn.ModuleList([nn.Linear(self.cur_dim, self.cur_dim, bias=False)
    ...
    def recur_aggregate(self, hidden, next_hidden, hidden_nodes, hidden_type, current_layer):
        if isinstance(hidden, list):
            ...
            for type_index in range(len(hidden)):
                ...
                updated_hidden, weight_p = self.recur_aggregate(neigh, next_neigh, next_hidden_nodes, next_type, current_layer)
            ...
            # if current_layer > 0:
            #     hidden_ = self.q_encoders[current_layer][hidden_type](hidden_)
            ...
            if current_layer > 0:
            # h_encoder = self.pre_encoders[current_layer][hidden_type]
            ...
            for type_index in range(len(next_hidden)):
                next_type = self.neighs_type[hidden_type][type_index]
                # neigh_encoder =
                # v_encoder = self.pre_encoders[current_layer][next_type]
                ...
                # neigh_hidden_ks.append(k_encoder(neigh_hidden_))
                # neigh_hidden_vs.append(v_encoder(neigh_hidden_))
            hidden_tmp = hidden_
            for clus_iter in range(self.n_iter):
                neigh_encodes = []
                for type_index in range(len(next_hidden)):
                    neigh_homo = self.homo_encoders[current_layer][hidden_type][type_index](weight_p[type_index], hidden_tmp, neigh_hiddens[type_index])
                    neigh_encodes.append(neigh_homo)
                hidden_tmp, weight_p = self.hete_encoders[current_layer][hidden_type](hidden_, torch.stack(neigh_encodes, dim=2))
            if current_layer > 0:
            ...
            hidden_shape.append(self.n_facet[current_layer])
    ...
    def recur_emb(self, neighs_layer, current_type):
        if isinstance(neighs_layer, list):
            ...
            # return self.emb[current_type][current_layer]
    ...
    def forward(self, user_neighs_layers, item_neighs_layers):
        ...
        # self.user_emb = user_hidden[0]
        # self.item_emb = item_hidden[0]
        ...
        for l in range(self.n_layer-1, -1, -1):
            user_h, user_weight_p = self.recur_aggregate(user_hidden[l], user_h, user_neighs_layers[l], 0, l)
            item_h, item_weight_p = self.recur_aggregate(item_hidden[l], item_h, item_neighs_layers[l], 1, l)
            self.user_weight_p.append(user_weight_p)
            self.item_weight_p.append(item_weight_p)
        logit = self.autocross(user_h, item_h)
        self.user_emb = user_h
        self.item_emb = item_h
        return logit
