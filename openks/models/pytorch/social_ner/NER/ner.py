...
def main():
    ...
    writer = SummaryWriter(
        logdir=os.path.join(args.output_dir, "eval"), comment="Linear"
    )
    ...
    if args.do_test:
        ...
        all_ori_labels = [
            ["O"] + e.label.split(" ") + ["O"] for e in test_examples
        ]
        # print("print test examples:\n")
        ...
        for b_i, (input_ids, input_mask, segment_ids, label_ids) in enumerate(
            ...
            # for l in label_ids:
            #     all_ori_labels.append([id2label[idx.item()] for idx in l])
        ...
        test_list = []
        with open(
            os.path.join(args.output_dir, "token_labels_.txt"),
            "w",
            encoding="utf-8",
        ) as f:
            for ori_tokens, ori_labels, prel in zip(
                all_ori_tokens, all_ori_labels, pred_labels
            ):
