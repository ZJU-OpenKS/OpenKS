...
class InputFeatures(object):
    ...
    def __init__(
        self, input_ids, input_mask, segment_ids, label_id, ori_tokens
    ):
            ...
            for item in jsonlines.Reader(fp):
                if item["entities"]:
                    for t in item["entities"]:
                        relations[t['type']] += 1 
        for re in relations.keys():
            new_list.append('B-'+re)
            new_list.append('I-'+re)
        return new_list
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in tqdm(
        enumerate(examples), desc="convert examples"
    ):
        # if ex_index % 10000 == 0:
        ...
        #     ntokens
        # ), f"{len(ori_tokens)}, {len(ntokens)}, {ori_tokens},{ntokens}"
