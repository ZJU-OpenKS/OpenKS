...
def evaluate(iterable, options=None):
    ...
    for line in iterable:
        ...
        if num_features is None:
            ...
            raise FormatError(
                "unexpected number of features: %d (%d)"
...
def report(counts, out=None):
    ...
    with open(DATA_Config.RESULT_TXT, "w", encoding="utf-8") as f:
        c = counts
        out.write(
        ...
        if c.token_counter > 0:
            ...
            out.write("Micro-precision: %6.2f%%; " % (100.0 * overall.prec))
            out.write("Micro-recall: %6.2f%%; " % (100.0 * overall.rec))
            out.write("Micro-F1: %6.2f\n" % (100.0 * overall.fscore))
            f.write("Micro-precision: %6.2f%%; " % (100.0 * overall.prec))
            f.write("Micro-recall: %6.2f%%; " % (100.0 * overall.rec))
            f.write("Micro-F1: %6.2f\n" % (100.0 * overall.fscore))
        ...
        for i, m in sorted(by_type.items()):
            ...
            f.write("%17s: " % i)
            f.write("precision: %6.2f%%; " % (100.0 * m.prec))
            f.write("recall: %6.2f%%; " % (100.0 * m.rec))
            f.write(
                "F1: %6.2f  %d\n" % (100.0 * m.fscore, c.t_found_guessed[i])
            )
            Macro_P += m.prec
        ...
        out.write("Macro-precision: %6.2f%%; " % (100.0 * (Macro_P / num)))
        out.write("Macro-recall: %6.2f%%; " % (100.0 * (Macro_R / num)))
        out.write("Macro-F1: %6.2f\n" % (100.0 * (Macro_F / num)))
        f.write("Macro-precision: %6.2f%%; " % (100.0 * (Macro_P / num)))
        f.write("Macro-recall: %6.2f%%; " % (100.0 * (Macro_R / num)))
        f.write("Macro-F1: %6.2f\n" % (100.0 * (Macro_F / num)))
...
def report_notprint(counts, out=None):
    ...
    if c.token_counter > 0:
        line = []
        line.append(
            "accuracy: %6.2f%%; " % (100.0 * c.correct_tags / c.token_counter)
        )
