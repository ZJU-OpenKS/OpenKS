...
# Modified by Yunfei Wei @ BUAA
# All rights reserved.
"""
The Relation Prediction Framework
"""
import numpy as np
import torch
...
class RelationPredictorOutputs(object):
    """
    Structure that holds Relation Predictor output object predicitions and relation predictions,
    ...
    def to(self, *args, **kwargs):
        ...
        return RelationPredictorOutputs(cast_predictions, cast_prediction_pairs)


SCENE_MODEL_DICT = ["sg_baseline", "sg_imp", "sg_msdn", "sg_grcnn", "sg_reldn", "sg_neuralmotif"]


class RelationPredictor(GeneralizedRCNN):
    """
    Scene Graph Generation model based on generalized RCNN.
    which mainly consists of 3 parts:
    - backbone
    - rpn
    - roi heads
    """
    ...
    def to(self, device, **kwargs):
        super(RelationPredictor, self).to(device, **kwargs)
    ...
    def forward(self, images, targets=None):
        ...
        if not self.detector_pre_calculated:
            ...
            if self.detector_force_boxes:
                ...
                x, predictions, detector_losses = self.roi_heads(features, proposals, targets)
            relation_losses.update(detector_losses)
        else:
            proposal_losses = {}
        ...
        # The predictions_pred contains idx_pairs (M*2) and scores (M*Pred_Cat) to rep triplets
        x_pairs, prediction_pairs, relation_losses = self.relation_head(features, predictions, targets)
        relation_losses.update(relation_losses)
        ...
        return [RelationPredictorOutputs(prediction, prediction_pair)
