...
class PoseResNet(nn.Module):
    ...
    def __init__(self, block, layers, heads, head_conv):
        ...
        self.tv_3 = nn.Sequential(*[nn.Conv1d(128, 128, kernel_size=3,padding=1, bias=False),
        nn.ReLU(inplace=True), 
        nn.Conv1d(128, 128, kernel_size=3,padding=1, bias=False),
        nn.ReLU(inplace=True),
        nn.Conv1d(128, 64, kernel_size=3,padding=1, bias=False)])

        self.tv_4 = nn.Sequential(*[nn.Conv1d(128, 128, kernel_size=3,padding=1, bias=False),
        nn.ReLU(inplace=True), 
        nn.Conv1d(128, 128, kernel_size=3,padding=1, bias=False),
        nn.ReLU(inplace=True),
        nn.Conv1d(128, 2, kernel_size=3,padding=1, bias=False)])
    ...
    def forward(self, x, sents, text_mask, test=False):
        ...
        if test:
            ...
            for i in range(sents.shape[1]):
                ...
                global_v = sequence_output_v.mean(-2).permute(0,2,1).contiguous()

                v1 = self.relu(self.ds1(global_v))

                global_v = self.relu(self.t1(global_v))

                v2 = self.relu(self.ds2(global_v))

                global_v = self.relu(self.t2(global_v))

                v3 = self.relu(self.ds3(global_v))

                global_v = self.relu(self.t3(global_v))

                global_v = self.relu(self.dt1(global_v)) + v3

                global_v = self.relu(self.dt2(global_v)) + v2

                global_v = self.relu(self.dt3(global_v)) + v1

                ret['relevance'].append(self.tv_3(global_v))

                ret['offset'].append(self.tv_4(global_v))
        ...
        global_t_1 = self.relu(self.mapping(sequence_output_t[:,:1,:]))
        ...
        # global_vt = torch.cat([global_v,global_t_1.expand_as(global_v)], dim=1)

        # ret['relevance'] = self.tv_3(global_v)

        ret['relevance'] = torch.bmm(global_t_1, self.tv_3(global_v))

        ret['offset'] = self.tv_4(global_v)
