...
class BertImageSelfAttention2(nn.Module):
    def __init__(self, config):
        super(BertImageSelfAttention2, self).__init__()
        if config.v_hidden_size % config.v_num_attention_heads != 0:
            raise ValueError(
                "The hidden size (%d) is not a multiple of the number of attention "
                "heads (%d)" % (config.v_hidden_size, config.v_num_attention_heads)
    ...
    def forward(self, hidden_states, attention_mask):
        hidden_states_spatial = hidden_states.mean(1)
        hidden_states_temporal = hidden_states.mean(2)
        hidden_states_full = torch.cat([hidden_states_spatial, hidden_states_temporal],dim=1)
        mixed_query_layer = self.query(hidden_states_full)
        mixed_key_layer = self.key(hidden_states_full)
        mixed_value_layer = self.value(hidden_states_full)
...
class BertImageSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        self.dense2 = nn.Linear(config.v_hidden_size, config.v_hidden_size)
        self.LayerNorm = BertLayerNorm(config.v_hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.v_hidden_dropout_prob)

    def forward(self, hidden_states1, hidden_states2, input_tensor):
        hidden_states1 = self.dense(hidden_states1)
        ...
        hidden_states = self.LayerNorm(hidden_states1.unsqueeze(1) + hidden_states2.unsqueeze(2) + input_tensor)
        return hidden_states

class BertImageSelfOutput2(nn.Module):
    def __init__(self, config):
        super(BertImageSelfOutput2, self).__init__()
        self.dense = nn.Linear(config.v_hidden_size, config.v_hidden_size)
        self.LayerNorm = BertLayerNorm(config.v_hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.v_hidden_dropout_prob)
    ...
    def forward(self, hidden_states, input_tensor):
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.LayerNorm(hidden_states[:,:input_tensor.size(2),:].unsqueeze(1)+ hidden_states[:,input_tensor.size(2):,:].unsqueeze(2) + input_tensor)
        return hidden_states
...
class BertImageAttention(nn.Module):
    def __init__(self, config):
        ...
        self.self2 = BertImageSelfAttention(config)
        self.output = BertImageSelfOutput(config)

    def forward(self, input_tensor, attention_mask1, attention_mask2):
        input_tensor_spatial = input_tensor.mean(1)
        input_tensor_temporal = input_tensor.mean(2)
        self_output1, attention_probs1 = self.self(input_tensor_spatial, attention_mask1)
        self_output2, attention_probs2 = self.self2(input_tensor_temporal, attention_mask2)
        attention_output = self.output(self_output1,self_output2, input_tensor)
        return attention_output, [attention_probs1, attention_probs2]
...
class BertImageLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask_spatial, attention_mask_temporal):
        # hidden_states_spatial = hidden_states.mean(1)
        # hidden_states_temporal = hidden_states.mean(2)
        attention_output, attention_probs = self.attention(hidden_states, attention_mask_spatial, attention_mask_temporal)
        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output(intermediate_output, attention_output)
        return layer_output, attention_probs
...
class BertBiAttention(nn.Module):
    def __init__(self, config):
        ...
        self.query2 = nn.Linear(config.v_hidden_size, self.all_head_size)
        self.key2 = nn.Linear(config.v_hidden_size, self.all_head_size)
        self.value2 = nn.Linear(config.v_hidden_size, self.all_head_size)

        self.query3 = nn.Linear(config.hidden_size, self.all_head_size)
        self.key3 = nn.Linear(config.v_hidden_size, self.all_head_size)
        self.value3 = nn.Linear(config.v_hidden_size, self.all_head_size)

        self.query4 = nn.Linear(config.v_hidden_size, self.all_head_size)
        self.key4 = nn.Linear(config.v_hidden_size, self.all_head_size)
        self.value4 = nn.Linear(config.v_hidden_size, self.all_head_size)

        self.dropout1 = nn.Dropout(config.v_attention_probs_dropout_prob)


        self.query5 = nn.Linear(config.hidden_size, self.all_head_size)
        self.key5 = nn.Linear(config.v_hidden_size, self.all_head_size)
        self.value5 = nn.Linear(config.v_hidden_size, self.all_head_size)

        self.query6 = nn.Linear(config.v_hidden_size, self.all_head_size)
        self.key6 = nn.Linear(config.v_hidden_size, self.all_head_size)
        self.value6 = nn.Linear(config.v_hidden_size, self.all_head_size)
    ...
    def forward(self, input_tensor1, attention_mask1, input_tensor2, attention_mask2, attention_mask3, co_attention_mask=None, use_co_attention_mask=False):

        # for vision input.
        input_tensor1_spatial = input_tensor1.mean(1)
        input_tensor1_temporal = input_tensor1.mean(2)
        # input_tensor1_full = torch.cat([input_tensor1_spatial, input_tensor1_temporal],dim=1)
        # print(attention_mask1.shape , attention_mask2.shape, attention_mask3.shape)
        mixed_query_layer1 = self.query1(input_tensor2)
        mixed_key_layer1 = self.key1(input_tensor1_spatial)
        mixed_value_layer1 = self.value1(input_tensor1_spatial)
        ...
        mixed_query_layer3 = self.query3(input_tensor2)
        mixed_key_layer3 = self.key3(input_tensor1_temporal)
        mixed_value_layer3 = self.value3(input_tensor1_temporal)
        # mixed_logit_layer1 = self.logit1(input_tensor1)
        ...
        # for text input:
        mixed_query_layer2 = self.query2(input_tensor1_spatial)
        mixed_key_layer2 = self.key2(input_tensor1_temporal)
        mixed_value_layer2 = self.value2(input_tensor1_temporal)
        ...
        attention_scores2 = attention_scores2 + attention_mask3
        if use_co_attention_mask:
        ...
        attention_scores3 = torch.matmul(query_layer3, key_layer3.transpose(-1, -2))
        attention_scores3 = attention_scores3 / math.sqrt(self.attention_head_size)
        # Apply the attention mask is (precomputed for all layers in BertModel forward() function)

        # we can comment this line for single flow. 
        attention_scores3 = attention_scores3 + attention_mask3
        if use_co_attention_mask:
            attention_scores2 = attention_scores2 + co_attention_mask
        ...
        # This is actually dropping out entire tokens to attend to, which might
        # seem a bit unusual, but is taken from the original Transformer paper.
        attention_probs3 = self.dropout1(attention_probs3)

        context_layer3 = torch.matmul(attention_probs3, value_layer3)
        ...
        mixed_query_layer4 = self.query4(input_tensor1_spatial)
        mixed_key_layer4 = self.key4(context_layer3)
        mixed_value_layer4 = self.value4(context_layer3)
        # mixed_logit_layer1 = self.logit1(input_tensor1)
        ...
        attention_scores4 = torch.matmul(query_layer4, key_layer4.transpose(-1, -2))
        attention_scores4 = attention_scores4 / math.sqrt(self.attention_head_size)
        # Apply the attention mask is (precomputed for all layers in BertModel forward() function)

        # we can comment this line for single flow. 
        attention_scores4 = attention_scores4 + attention_mask2
        if use_co_attention_mask:
            attention_scores2 = attention_scores2 + co_attention_mask

        # Normalize the attention scores to probabilities.
        attention_probs4 = nn.Softmax(dim=-1)(attention_scores4)

        # This is actually dropping out entire tokens to attend to, which might
        # seem a bit unusual, but is taken from the original Transformer paper.
        attention_probs4 = self.dropout2(attention_probs4)

        context_layer4 = torch.matmul(attention_probs4, value_layer4)
        ...
        mixed_query_layer5 = self.query5(input_tensor2)
        mixed_key_layer5 = self.key5(context_layer2)
        mixed_value_layer5 = self.value5(context_layer2)
        # mixed_logit_layer2 = self.logit2(input_tensor2)

        query_layer5 = self.transpose_for_scores(mixed_query_layer5)
        key_layer5 = self.transpose_for_scores(mixed_key_layer5)
        value_layer5 = self.transpose_for_scores(mixed_value_layer5)
        # logit_layer2 = self.transpose_for_logits(mixed_logit_layer2)

        attention_scores5 = torch.matmul(query_layer5, key_layer5.transpose(-1, -2))
        attention_scores5 = attention_scores5 / math.sqrt(self.attention_head_size)
        # Apply the attention mask is (precomputed for all layers in BertModel forward() function)

        # we can comment this line for single flow. 
        attention_scores5 = attention_scores5 + attention_mask1
        if use_co_attention_mask:
            attention_scores2 = attention_scores2 + co_attention_mask

        # Normalize the attention scores to probabilities.
        attention_probs5 = nn.Softmax(dim=-1)(attention_scores5)

        # This is actually dropping out entire tokens to attend to, which might
        # seem a bit unusual, but is taken from the original Transformer paper.
        attention_probs5 = self.dropout1(attention_probs5)

        context_layer5 = torch.matmul(attention_probs5, value_layer5)
        context_layer5 = context_layer5.permute(0, 2, 1, 3).contiguous()
        new_context_layer_shape5 = context_layer5.size()[:-2] + (self.all_head_size,)
        context_layer5 = context_layer5.view(*new_context_layer_shape5)

        mixed_query_layer6 = self.query6(input_tensor1_temporal)
        mixed_key_layer6 = self.key6(context_layer1)
        mixed_value_layer6 = self.value6(context_layer1)
        # mixed_logit_layer2 = self.logit2(input_tensor2)

        query_layer6 = self.transpose_for_scores(mixed_query_layer6)
        key_layer6 = self.transpose_for_scores(mixed_key_layer6)
        value_layer6 = self.transpose_for_scores(mixed_value_layer6)
        # logit_layer2 = self.transpose_for_logits(mixed_logit_layer2)

        attention_scores6 = torch.matmul(query_layer6, key_layer6.transpose(-1, -2))
        attention_scores6 = attention_scores6 / math.sqrt(self.attention_head_size)
        # Apply the attention mask is (precomputed for all layers in BertModel forward() function)

        # we can comment this line for single flow. 
        attention_scores6 = attention_scores6 + attention_mask2
        if use_co_attention_mask:
            attention_scores2 = attention_scores2 + co_attention_mask

        # Normalize the attention scores to probabilities.
        attention_probs6 = nn.Softmax(dim=-1)(attention_scores6)

        # This is actually dropping out entire tokens to attend to, which might
        # seem a bit unusual, but is taken from the original Transformer paper.
        attention_probs6 = self.dropout2(attention_probs6)

        context_layer6 = torch.matmul(attention_probs6, value_layer6)
        context_layer6 = context_layer6.permute(0, 2, 1, 3).contiguous()
        new_context_layer_shape6 = context_layer6.size()[:-2] + (self.all_head_size,)
        context_layer6 = context_layer6.view(*new_context_layer_shape6)



        return context_layer4, context_layer5, context_layer6, (attention_probs4, attention_probs5, attention_probs6)
...
class BertBiOutput(nn.Module):
    def __init__(self, config):
        ...
        self.dense3 = nn.Linear(config.bi_hidden_size, config.v_hidden_size)
        # self.LayerNorm3 = BertLayerNorm(config.v_hidden_size, eps=1e-12)
        self.dropout3 = nn.Dropout(config.v_hidden_dropout_prob)

        # self.q_dense3 = nn.Linear(config.bi_hidden_size, config.v_hidden_size)
        # self.q_dropout3 = nn.Dropout(config.v_hidden_dropout_prob)

    def forward(self, hidden_states1, input_tensor1, hidden_states2, input_tensor2,hidden_states3):
        ...
        hidden_states1 = self.LayerNorm1(context_state1.unsqueeze(1) + context_state3.unsqueeze(2) + input_tensor1)
...
class BertConnectionLayer(nn.Module):
    def __init__(self, config):
        ...
        # self.v_intermediate2 = BertImageIntermediate(config)
        # self.v_output2 = BertImageOutput(config)
    ...
    def forward(self, input_tensor1, attention_mask1, input_tensor2, attention_mask2, attention_mask3, co_attention_mask=None, use_co_attention_mask=False):
        ...
        bi_output1, bi_output2, bi_output3, co_attention_probs = self.biattention(
            input_tensor1, attention_mask1, input_tensor2, attention_mask2,attention_mask3, co_attention_mask, use_co_attention_mask
        )

        attention_output1, attention_output2 = self.biOutput(bi_output1, input_tensor1, bi_output2, input_tensor2, bi_output3)
        ...
        # intermediate_output3 = self.v_intermediate2(attention_output3)
        # layer_output3 = self.v_output2(intermediate_output3, attention_output3)
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        image_embedding_spatial = image_embedding.mean(1)
        image_embedding_temporal = image_embedding.mean(2)

        image_attention_mask_spatial = image_attention_mask[:,:,:,:num_regions]
        image_attention_mask_temporal = image_attention_mask[:,:,:,num_regions:]
        # print(image_attention_mask_spatial.shape, image_attention_mask_temporal.shape)
        ...
        for v_layer_id, t_layer_id in zip(self.v_biattention_id, self.t_biattention_id):
            ...
            for idx in range(v_start, self.fixed_v_layer):
                with torch.no_grad():
                    image_embedding, image_attention_probs = self.v_layer[idx](image_embedding, image_attention_mask_spatial, image_attention_mask_temporal)
                    v_start = self.fixed_v_layer
                    # image_embedding_temporal, image_attention_probs_temporal = self.v_layer2[idx](image_embedding_temporal, image_attention_mask_temporal)
            ...
            if self.with_coattention:
                ...
                image_embedding, txt_embedding, co_attention_probs = self.c_layer[count](
                    image_embedding, image_attention_mask_spatial, txt_embedding, txt_attention_mask,image_attention_mask_temporal, co_attention_mask, use_co_attention_mask)
        ...
        if not output_all_encoded_layers:
            # image_embedding += image_embedding_spatial.unsqueeze(1) + image_embedding_temporal.unsqueeze(2) 
            all_encoder_layers_t.append(txt_embedding)
            all_encoder_layers_v.append(image_embedding)
    ...
    def forward(
        ...
        if image_attention_mask is None:
            image_attention_mask = torch.ones(
                input_imgs.size(0), input_imgs.size(1)+input_imgs.size(2)
            ).type_as(input_txt)
        ...
        # print(input_txt.size(), input_imgs.size())
        if co_attention_mask is None:
            co_attention_mask = torch.zeros(input_txt.size(0), input_imgs.size(1)+input_imgs.size(2), input_txt.size(1)).type_as(extended_image_attention_mask)         
...
class BertImageEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.image_location_embeddings2 = nn.Linear(6, config.v_hidden_size)
        self.LayerNorm = BertLayerNorm(config.v_hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def forward(self, input_ids, input_loc):
        ...
        loc_embeddings = self.image_location_embeddings2(input_loc)        
...
class VILBertForVGROUND(BertPreTrainedModel):
    def __init__(self, config, dropout_prob=0.1, default_gpu=True):
        super(VILBertForVGROUND, self).__init__(config)
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(dropout_prob)
        self.apply(self.init_bert_weights)
