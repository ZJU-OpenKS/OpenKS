...
class BertBiAttention(nn.Module):
    def __init__(self, config):
        ...
        self.query3 = nn.Linear(config.v_hidden_size, self.all_head_size)
        self.key3 = nn.Linear(config.v_hidden_size, self.all_head_size)
        self.value3 = nn.Linear(config.v_hidden_size, self.all_head_size)

        self.convert_2_t1 = nn.Linear(self.all_head_size, config.hidden_size)
        self.convert_2_t2 = nn.Linear(self.all_head_size, config.hidden_size)

        # self.query4 = nn.Linear(config.v_hidden_size, self.all_head_size)
        self.key4 = nn.Linear(config.hidden_size, self.all_head_size)
        self.value4 = nn.Linear(config.hidden_size, self.all_head_size)
    ...
    def forward(self, input_tensor1, attention_mask1, input_tensor2, attention_mask2, input_tensor3, attention_mask3, co_attention_mask=None, use_co_attention_mask=False):
        ...
        context_layer1 = self.convert_2_t1(context_layer1.view(*new_context_layer_shape1))


        mixed_key_layer2 = self.key2(context_layer1)
        mixed_value_layer2 = self.value2(context_layer1)
        ...
        attention_scores2 = torch.matmul(query_layer1, key_layer3.transpose(-1, -2))
        ...
        context_layer2 = torch.matmul(attention_probs2, value_layer3)
        context_layer2 = context_layer2.permute(0, 2, 1, 3).contiguous()
        new_context_layer_shape2 = context_layer2.size()[:-2] + (self.all_head_size,)
        context_layer2 = context_layer2.view(*new_context_layer_shape2)


        attention_scores3 = torch.matmul(query_layer2, key_layer3.transpose(-1, -2))
        ...
        context_layer3 = self.convert_2_t2(context_layer3.view(*new_context_layer_shape3))


        # mixed_query_layer4 = self.query4(input_tensor1)
        ...
        attention_scores4 = torch.matmul(query_layer1, key_layer4.transpose(-1, -2))
        ...
        attention_scores5 = torch.matmul(query_layer2, key_layer5.transpose(-1, -2))
        ...
        # mixed_query_layer6 = self.query6(input_tensor3)
        ...
        attention_scores6 = torch.matmul(query_layer3, key_layer2.transpose(-1, -2))
        ...
        context_layer6 = torch.matmul(attention_probs6, value_layer2)
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.LayerNorm1 = nn.ModuleList([BertLayerNorm(config.v_hidden_size, eps=1e-12) for _ in range(len(config.v_biattention_id))])
        self.LayerNorm2 = nn.ModuleList([BertLayerNorm(config.v_hidden_size, eps=1e-12) for _ in range(len(config.v_biattention_id))])
    ...
    def forward(
        ...
        for v_layer_id, t_layer_id in zip(self.v_biattention_id, self.t_biattention_id):
            ...
            for idx in range(v_start, self.fixed_v_layer):
                with torch.no_grad():
                    image_embedding_spatial, image_attention_probs = self.v_layer[idx](image_embedding_spatial, image_attention_mask_spatial)
            ...
            # image_embedding = self.LayerNorm1[count](image_embedding + image_embedding_spatial.unsqueeze(1) + image_embedding_temporal.unsqueeze(2))
            ...
            if self.with_coattention:
                ...
                image_embedding_spatial, txt_embedding, image_embedding_temporal, co_attention_probs = self.c_layer[count](
                    image_embedding_spatial, image_attention_mask_spatial, txt_embedding, txt_attention_mask, image_embedding_temporal,image_attention_mask_temporal, co_attention_mask, use_co_attention_mask)
            ...
            # image_embedding = self.LayerNorm2[count](image_embedding + image_embedding_spatial.unsqueeze(1) + image_embedding_temporal.unsqueeze(2))
        ...
        if not output_all_encoded_layers:
            image_embedding = image_embedding + image_embedding_spatial.unsqueeze(1) + image_embedding_temporal.unsqueeze(2)
