...
class BertImageSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, attention_probs_spatial, attention_probs_temporal):
        hidden_states_spatial = (hidden_states.mean(1)) * (attention_probs_spatial.squeeze(1))
        hidden_states_temporal = hidden_states.mean(2)
        # hidden_states_spatial = (hidden_states * attention_probs_temporal).sum(1)
        # hidden_states_temporal = (hidden_states * attention_probs_spatial).sum(2)
...
class BertImageSelfOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor, attention_probs_spatial, attention_probs_temporal):
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.LayerNorm(hidden_states[:,:input_tensor.size(2),:].unsqueeze(1) * attention_probs_spatial + hidden_states[:,input_tensor.size(2):,:].unsqueeze(2) + input_tensor)
...
class BertImageAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, attention_probs_spatial, attention_probs_temporal):
        self_output, attention_probs = self.self(input_tensor, attention_mask, attention_probs_spatial, attention_probs_temporal)
        attention_output = self.output(self_output, input_tensor, attention_probs_spatial, attention_probs_temporal)
        return attention_output, attention_probs
...
class BertImageLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, attention_probs_spatial, attention_probs_temporal):
        attention_output, attention_probs = self.attention(hidden_states, attention_mask, attention_probs_spatial, attention_probs_temporal)
        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output(intermediate_output, attention_output)
        return layer_output, attention_probs
...
class BertBiAttention(nn.Module):
    ...
    def forward(self, input_tensor1, attention_mask1, input_tensor2, attention_mask2, attention_probs_spatial, attention_probs_temporal, co_attention_mask=None, use_co_attention_mask=False):

        # for vision input.
        input_tensor1_spatial = (input_tensor1.mean(1)) * (attention_probs_spatial.squeeze(1))
        input_tensor1_temporal = input_tensor1.mean(2)
        # input_tensor1_spatial = (input_tensor1 * attention_probs_temporal).sum(1)
        # input_tensor1_temporal = (input_tensor1 * attention_probs_spatial).sum(2)
        input_tensor1_full = torch.cat([input_tensor1_spatial, input_tensor1_temporal],dim=1)
        mixed_query_layer1 = self.query1(input_tensor1_full)
        mixed_key_layer1 = self.key1(input_tensor1_full)
        mixed_value_layer1 = self.value1(input_tensor1_full)
        ...
        text_mask = (attention_mask2/10000.0 + 1.0).permute(0,1,3,2)
        # Normalize the attention scores to probabilities.
        attention_probs1 = nn.Softmax(dim=-1)(attention_scores1)
        attention_probs_spatial = nn.Softmax(dim=-1)((attention_scores1[:,:,:,:input_tensor1.size(2)]*text_mask).sum(dim=1,keepdim=True).sum(dim=2,keepdim=True))
        attention_probs_spatial = attention_probs_spatial.permute(0,1,3,2).contiguous()
        attention_probs_temporal = nn.Softmax(dim=-1)(attention_scores1[:,:,:,input_tensor1.size(2):].sum(dim=1,keepdim=True).sum(dim=2,keepdim=True))
        attention_probs_temporal = attention_probs_temporal.permute(0,3,1,2).contiguous()
        ...
        return context_layer1, context_layer2, (attention_probs1, attention_probs2), attention_probs_spatial, attention_probs_temporal
...
class BertBiOutput(nn.Module):
    ...
    def forward(self, hidden_states1, input_tensor1, hidden_states2, input_tensor2, attention_probs_spatial, attention_probs_temporal):
        ...
        hidden_states1 = self.LayerNorm1(context_state1[:,:input_tensor1.size(2),:].unsqueeze(1) * attention_probs_spatial + context_state1[:,input_tensor1.size(2):,:].unsqueeze(2) + input_tensor1)
        # hidden_states1 = self.LayerNorm1(context_state1[:,:input_tensor1.size(2),:].unsqueeze(1) + context_state1[:,input_tensor1.size(2):,:].unsqueeze(2) + input_tensor1)
...
class BertConnectionLayer(nn.Module):
    ...
    def forward(self, input_tensor1, attention_mask1, input_tensor2, attention_mask2, attention_probs_spatial, attention_probs_temporal, co_attention_mask=None, use_co_attention_mask=False):
        ...
        bi_output1, bi_output2, co_attention_probs, attention_probs_spatial, attention_probs_temporal = self.biattention(
            input_tensor1, attention_mask1, input_tensor2, attention_mask2, attention_probs_spatial, attention_probs_temporal, co_attention_mask, use_co_attention_mask
        )

        attention_output1, attention_output2 = self.biOutput(bi_output2, input_tensor1, bi_output1, input_tensor2, attention_probs_spatial, attention_probs_temporal)
        ...
        return layer_output1, layer_output2, co_attention_probs, attention_probs_spatial, attention_probs_temporal
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        _,num_frames, num_regions, v_hidden_size = image_embedding.size()

        attention_probs_spatial = torch.ones(batch_size, 1, num_regions, 1).type_as(image_embedding) /num_regions 
        attention_probs_temporal = torch.ones_like(image_embedding)/num_frames
        ...
        for v_layer_id, t_layer_id in zip(self.v_biattention_id, self.t_biattention_id):
            ...
            for idx in range(v_start, self.fixed_v_layer):
                with torch.no_grad():
                    image_embedding, image_attention_probs = self.v_layer[idx](image_embedding, image_attention_mask, attention_probs_spatial, attention_probs_temporal)
            ...
            if self.with_coattention:
                # do the bi attention.
                image_embedding, txt_embedding, co_attention_probs, attention_probs_spatial, attention_probs_temporal = self.c_layer[count](
                    image_embedding, image_attention_mask, txt_embedding, txt_attention_mask, attention_probs_spatial, attention_probs_temporal, co_attention_mask, use_co_attention_mask)
