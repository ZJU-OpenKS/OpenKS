...
for img_idx, img_info in enumerate(lines):
    ...
    if pid_num < 47146:
        ...
        pid_train_list.append(str(pid_sum))
        #print("img_path", img_path)
        #print("pid_sum", pid_sum)
    else:
        if pid_now != pid_num:
            pid_sum += 1
        img_path_test_list.append(img_path)
        pid_test_list.append(str(pid_sum))
        if pid_now != pid_num:
            pid_now = pid_num
            img_path_query_list.append(img_path)
            pid_query_list.append(str(pid_sum))
        else:
            img_path_gallery_list.append(img_path)
            pid_gallery_list.append(str(pid_sum))
