...
from models import CitationGeneration
from data import S2orcDataset
...
def parse_args():
    ...
    parser.add_argument('--dataset', default='Computer_Science',
...
def inference(model, test_data_loader, index2vocab, device):
    print('Start Test')
    bleu_score, rouge_score, f1_micro, f1_macro, hl, report, diversity = decode(model, test_data_loader, index2vocab, device)
    print('Test:')
    print('\tbleu-1:{0[0]}, bleu-2:{0[1]}, bleu-3:{0[2]}, bleu-4:{0[3]}'.format(bleu_score))
    print('\trouge-1-f:{0[0]}, rouge-2-f:{0[1]}, rouge-l-f:{0[2]}'.format(rouge_score))
    print('f1_micro:{}, f1_macro:{}, hl:{}'.format(f1_micro, f1_macro, hl))
...
def decode(model, data_loader, index2vocab, device):
    ...
    return bleu_score, rouge_score, f1_micro, f1_macro, hl, report, diversity
...
def main():
    ...
    with open('./data/processed/heter_graph.pkl','wb') as fp:
        pickle.dump(heter_graph, fp)
