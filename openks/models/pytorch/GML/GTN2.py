...
#     def __init__(self, cur_dim, n_head_layer, n_head_cross, dropout, device):
...
#             nn.Linear(cur_dim, cur_dim),
#         )
#         self.norm1 = nn.LayerNorm(cur_dim).to(device)
...
#         output_att = self.norm1(h_feature + self.dropout(output_att))
...
class GTN(nn.Module):
    def __init__(self, cur_dim, n_neighs, n_head_layer, n_head_cross, n_cross, dropout, device):
        ...
        #     [CrossLayer(cur_dim, n_head_layer, n_head_cross, dropout, device) for _ in range(self.n_cross)]).to(device)
    ...
    def forward(self, tgt_feature, neigh_features):
        ...
        h_list = h_list.view(self.n_layer+1, -1, self.cur_dim) * math.sqrt(self.cur_dim)
        ...
        # h_feature = torch.cat(h_list.chunk(self.n_layer+1, dim=0), dim=-1).squeeze(0)
        return h_feature
