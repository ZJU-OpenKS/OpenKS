...
def evaluate_embedding(embeddings, labels, search=True): # True, False
    ...
    return np.mean(svc_accuracies) #, np.mean(logreg_accuracies),
           # np.mean(linearsvc_accuracies), np.mean(randomforest_accuracies)
