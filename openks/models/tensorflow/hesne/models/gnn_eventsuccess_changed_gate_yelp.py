...
class GnnEventModel_changed_gate(BasicModel):
    ...
    def get_log_file(self, params):
        log_file = 'h_' + str(self.params['hidden_size']) + '_b_' + str(self.params['batch_event_numbers']) \
                    ...
                    + str(self.params['decay_step']) + '_k_' + str(self.params['keep_prob']) + 'changed_gate'
        return log_file
    ...
    def make_model(self):
        ...
        with tf.variable_scope('graph_model'):
            ...
            self.ops['loss'], self.ops['last_time'] = self.build_specific_graph_model()
            self.ops['loss_eval'], self.ops['predict'], self.ops['neg_predict'], self.ops['node_vec'], \
                self.ops['node_cel'], self.ops['last_time_eval'] = self.build_specific_eval_graph_model()
...
######################################################
    ...
    def build_specific_graph_model(self):
        ...
        for event_id in range(self._eventnum_batch):
            ...
            # event_states_st = tf.zeros([self._h_dim], dtype=tf.float64)
            # event_states_dy = tf.zeros([self._h_dim], dtype=tf.float64)
            # neg_event_states_st = [tf.zeros([self._h_dim], dtype=tf.float64) for _ in range(self._neg_num)]
            # neg_event_states_dy = [tf.zeros([self._h_dim], dtype=tf.float64) for _ in range(self._neg_num)]
            neg_event_states = [tf.zeros([self._h_dim], dtype=tf.float64) for _ in range(self._neg_num)]
            # neg_concate_event_states = [tf.zeros([self._h_dim], dtype=tf.float64) for _ in range(self._neg_num)]
            for node_type in range(self._type_num):
                ...
                states_aggregated = tf.reduce_mean(states)
                # states_aggregated_st = tf.reduce_mean(states_st, axis=0, keepdims=True)
                # states_aggregated_dy = tf.reduce_mean(states_dy, axis=0, keepdims=True)
                ...
                # event_states_st += tf.scalar_mul(self.weights['type_weights_scalar'][node_type], states_aggregated_st)
                # event_states_dy += tf.scalar_mul(self.weights['type_weights_scalar'][node_type], states_aggregated_dy)
            # concate_event_states = tf.concat([event_states_st, event_states_dy], 1)
            # event_states = self.dense_layer(concate_event_states)
            ...
            for neg in range(self._neg_num):
                for node_type in range(self._type_num):
                    ...
                    # neg_states_aggregated_st = tf.reduce_mean(neg_states_st, axis=0, keepdims=True)
                    # neg_states_aggregated_dy = tf.reduce_mean(neg_states_dy, axis=0, keepdims=True)
                    ...
                    # neg_event_states_st[neg] += tf.scalar_mul(self.weights['type_weights_scalar'][node_type], neg_states_aggregated_st)
                    # neg_event_states_dy[neg] += tf.scalar_mul(self.weights['type_weights_scalar'][node_type], neg_states_aggregated_dy)

                # neg_event_states[neg] = self.activation_fun(tf.matmul(neg_event_states[neg], self.weights['event_weights'])) + neg_event_states[neg]
                # neg_concate_event_states[neg] = tf.concat([neg_event_states_st[neg], neg_event_states_dy[neg]], 1)
                # neg_event_states[neg] = self.dense_layer(neg_concate_event_states[neg])
            ...
            for node_type in range(self._type_num):
                ...
                target_embedding = states_dy

                # sent_states_st = tf.tile(event_states_st, [tf.shape(target_embedding)[0], 1])
                # sent_states_dy = tf.tile(event_states_dy, [tf.shape(target_embedding)[0], 1])
                ...
                sent_states = self.timedyn_layer([delta_time, states_st, states_dy, sent_states])
        ...
        return loss_mean, node_last_time[event_id+1]
    ...
    def build_specific_eval_graph_model(self):
        ...
        neg_event_states_st = tf.zeros([self._h_dim], dtype=tf.float64)
        neg_event_states_dy = tf.zeros([self._h_dim], dtype=tf.float64)
        for node_type in range(self._type_num):
            ...
            neg_event_states_st += tf.scalar_mul(self.weights['type_weights_scalar'][node_type], neg_states_aggregated_st)
            neg_event_states_dy += tf.scalar_mul(self.weights['type_weights_scalar'][node_type], neg_states_aggregated_dy)
        # concate_event_states = tf.concat([event_states_st, event_states_dy], 1)
        # event_states = self.dense_layer(concate_event_states)
        # neg_concate_event_states = tf.concat([neg_event_states_st, neg_event_states_dy], 1)
        # neg_event_states = self.dense_layer(neg_concate_event_states)
        ...
        for node_type in range(self._type_num):
            ...
            event_time = tf.tile(tf.expand_dims(self.placeholders['events_time_eval_ph'], 1),
                                 [tf.shape(last_time)[0], 1])
            delta_time = event_time - last_time
            sent_states = self.timedyn_layer([delta_time, states_st, states_dy, sent_states_st, sent_states_dy])
        ...
        with open(self.log_file, 'w') as log_out:
            for epoch in range(self.params['num_epochs']):
                ...
                while not epoch_flag:
                    ...
                    fetches = [self.ops['loss'], self.ops['last_time'], self.ops['global_step'], self.ops['lr'], self.ops['train_op']]
                    cost, self.last_time, step, lr, _ = self.sess.run(fetches, feed_dict=batch_feed_dict)
    ...
    def validation(self):
        ...
        while not epoch_flag:
            fetches = [self.ops['loss_eval'], self.ops['node_vec'], self.ops['node_cel'], self.ops['last_time_eval']]
            feed_dict_valid, epoch_flag = self.get_feed_dict_eval('valid')
            cost, self.node_embedding_evalcur, self.node_cellstates_evalcur, self.last_time = self.sess.run(fetches, feed_dict=feed_dict_valid)
            valid_loss.append(cost)
        return np.mean(valid_loss)
    ...
    def test(self):
        ...
        while not epoch_flag:
            ...
            fetches = [self.ops['predict'], self.ops['neg_predict'], self.ops['node_vec'], self.ops['node_cel'], self.ops['last_time_eval']]
            feed_dict_test, epoch_flag = self.get_feed_dict_eval('test')
            predict, neg_predict, self.node_embedding_evalcur, self.node_cellstates_evalcur, self.last_time = \
