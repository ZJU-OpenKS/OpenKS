...
class GnnEventModel_changed_new_eventgate(BasicModel):
    ...
    def _create_placeholders(self):
            ...
            self.placeholders['is_neighbor'] = [tf.placeholder(tf.bool, name='is_neighbor') for event in range(self._eventnum_batch)]
...
######################################################
    ...
    def build_specific_graph_model(self):
        ...
        # def event_propagation(node_vec, node_cel, event_states, neighbor_type, partition):
        ...
        #     #                         self.placeholders['neighbor_partition_idx_ph'][event_id], 2)
        #     node_cel_part = tf.dynamic_partition(node_cel, partition, 2)
        #     node_vec_part = tf.dynamic_partition(node_vec, partition, 2)
        #     new_neighbor_celstates, new_neighbor_embedding = self.event_layer(
        #         [tf.tile(event_states, [tf.shape(node_cel_part[1])[0], 1]),
        #          node_vec_part[1], node_cel_part[1], neighbor_type])
        ...
        #     #                                         self.placeholders['neighbor_partition_idx_ph'][event_id], 2)
        #     neighbor_condition_indices = tf.dynamic_partition(tf.range(self._num_node), partition, 2)
        #
        ...
        def event_propagation(event_id, node_vec, node_cel, event_states, new_target_embedding):
            ...
            new_neighbor_embedding, new_neighbor_celstates = self.event_layer([event_states,
        ...
        for event_id in range(self._eventnum_batch):
            ...
            for node_type in range(self._type_num):
                ...
                # new_target_embedding.append(target_embedding)
                # new_target_celstates.append(target_celstates)
            ...
            # type_wt = tf.diag(type_w)
            # send_embedding = new_target_embedding[1:]
            # neighbor_embedding = tf.dynamic_partition(node_vec[event_id+1], self.placeholders['neighbor_partition_idx_ph'][event_id], 2)
            # neighbor_celstates = tf.dynamic_partition(node_cel[event_id+1], self.placeholders['neighbor_partition_idx_ph'][event_id], 2)
            # target_info = tf.matmul(type_wt, send_embedding[1])
            # target_input = tf.sparse_tensor_dense_matmul(self.placeholders['cur_adj_ph'][event_id], target_info)
            # new_neighbor_embedding, new_neighbor_celstates = self.event_layer(event_states, target_input, neighbor_embedding, neighbor_celstates)
            # neighbor_condition_indices = tf.dynamic_partition(tf.range(self._num_node),
            #                                         self.placeholders['neighbor_partition_idx_ph'][event_id], 2)
            # node_vec[event_id+1] = tf.dynamic_stitch(neighbor_condition_indices, [neighbor_embedding[0], new_neighbor_embedding])
            # node_cel[event_id+1] = tf.dynamic_stitch(neighbor_condition_indices, [neighbor_celstates[0], new_neighbor_celstates])
            ####changed


            # type_ds = tf.sparse.reduce_sum(self.placeholders['cur_adj_ph'][event_id], 0)
            # type_d = tf.diag(tf.reciprocal(type_ds))
            # new_target_embedding = tf.nn.embedding_lookup(node_vec[event_id+1], event_nodes)
            # new_target_embedding = tf.matmul(type_wt, new_target_embedding)
            # # new_target_celstates = tf.matmul(type_d, new_target_celstates)
            # new_neighbor_embedding = tf.sparse_tensor_dense_matmul(self.placeholders['cur_adj_ph'][event_id], new_target_embedding)

            # node_vec[event_id+1], node_cel[event_id+1] = tf.cond(self.placeholders['is_neighbor'][event_id],
            #                     lambda: event_propagation(node_vec[event_id+1], node_cel[event_id+1], event_states, self.placeholders['neighbor_type'][event_id],self.placeholders['neighbor_partition_idx_ph'][event_id]),
    ...
    def build_specific_eval_graph_model(self):
        ...
        # # type_d = tf.diag(tf.reciprocal(type_ds))
        # new_target_embedding = tf.nn.embedding_lookup(node_vec, event_nodes)
        # new_target_embedding = tf.matmul(type_wt, new_target_embedding)
        # # new_target_celstates = tf.matmul(type_d, new_target_celstates)
        # new_neighbor_embedding = tf.sparse_tensor_dense_matmul(self.placeholders['cur_adj_eval_ph'], new_target_embedding)
        ...
        #     new_neighbor_celstates, new_neighbor_embedding = self.event_layer([tf.tile(event_states, [tf.shape(node_cel_part[1])[0], 1]),
        #                                                node_vec_part[1], node_cel_part[1]])
        #
        #     neighbor_condition_indices = tf.dynamic_partition(tf.range(self._num_node),
        #                                                       self.placeholders['neighbor_partition_idx_eval_ph'], 2)
        #
        #     node_cel = tf.dynamic_stitch(neighbor_condition_indices,
        #                                                [node_cel_part[0], new_neighbor_celstates])
        #     node_vec = tf.dynamic_stitch(neighbor_condition_indices,
        #                                                [node_vec_part[0], new_neighbor_embedding])
        #     return node_vec, node_cel
        #
        # def no_propagation():
        ...
        # type_wt = tf.diag(type_w)
        # send_embedding = new_target_embedding[1:]
        # neighbor_embedding = tf.dynamic_partition(node_vec,
        #                                           self.placeholders['neighbor_partition_idx_eval_ph'], 2)
        # neighbor_celstates = tf.dynamic_partition(node_cel,
        #                                           self.placeholders['neighbor_partition_idx_eval_ph'], 2)
        # target_info = tf.matmul(type_wt, send_embedding[1])
        # target_input = tf.sparse_tensor_dense_matmul(self.placeholders['cur_adj_eval_ph'], target_info)
        # new_neighbor_embedding, new_neighbor_celstates = self.event_layer(event_states, target_input,
        #                                                                   neighbor_embedding, neighbor_celstates)
        # neighbor_condition_indices = tf.dynamic_partition(tf.range(self._num_node),
        #                                                   self.placeholders['neighbor_partition_idx_eval_ph'], 2)
        # node_vec = tf.dynamic_stitch(neighbor_condition_indices,
        #                                            [neighbor_embedding[0], new_neighbor_embedding])
        # node_cel = tf.dynamic_stitch(neighbor_condition_indices,
        #                                            [neighbor_celstates[0], new_neighbor_celstates])
        #########changed

        # node_vec, node_cel = tf.cond(self.placeholders['is_neighbor_eval'],
        #                              lambda: event_propagation(node_vec, node_cel, event_states, self.placeholders['neighbor_type_eval'], self.placeholders['neighbor_partition_idx_eval_ph']),
        ...
        with open(self.log_file, 'w') as log_out:
            for epoch in range(self.params['num_epochs']):
                ...
                while not epoch_flag:
                    ...
                    fetches = [self.ops['loss'], self.ops['test0'], self.ops['test1'], self.ops['node_vec'], self.ops['node_cel'], self.ops['node_last_time'],\
                               self.ops['global_step'], self.ops['lr'], self.ops['train_op']]
                    cost,test0, test1, self.node_embedding_cur, self.node_cellstates_cur, self.node_last_time, step, lr, _ =\
    ...
    def validation(self):
        ...
        while not epoch_flag:
            ...
            cost, self.node_embedding_cur, self.node_cellstates_cur, self.node_last_time, test0, test1 = self.sess.run(fetches, feed_dict=feed_dict_valid)
            # if num%10==0:
            #     print(test0)
            #     print('*********')
            #     print(test1)
    ...
    def get_batch_feed_dict(self, state, is_init):
        ...
        ###############feed neighbor###################
            ...
            # for neg in range(self._neg_num):
            #     print(batch_data_neg_list[neg][event])
            # time.sleep(2)
