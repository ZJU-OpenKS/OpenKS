...
class GnnEventModel_withattention(BasicModel):
    ...
    def get_log_file(self):
        log_file = 'h_' + str(self.params['hidden_size']) + '_b_' + str(self.params['batch_event_numbers']) \
                    ...
                    + str(self.params['max_his_num']) + '_n_' + str(self.params['negative_ratio']) + '_ns_' + str(self.params['negative_sampling']) + '_test.log'
        return log_file
    ...
    def get_checkpoint_dir(self):
        checkpoint_dir = 'h_' + str(self.params['hidden_size']) + '_b_' + str(self.params['batch_event_numbers']) \
                    + '_l_' + str(self.params['learning_rate']) + '_d_' + str(self.params['decay']) + '_ds_' \
                    + str(self.params['decay_step']) + '_k_' + str(self.params['keep_prob']) + '_'+str(self.params['his_type'])+'_'\
                    + str(self.params['max_his_num']) + '_n_' + str(self.params['negative_ratio']) + '_ns_' + str(self.params['negative_sampling']) + '_test'
    ...
    def make_model(self):
        with tf.variable_scope('graph_model'):
            ...
            self.ops['loss'], self.ops['node_vec'], self.ops['node_cel'], self.ops['node_his'], self.ops['pred'], \
                                                    self.ops['neg_pred'], self.ops['train_sample'] = self.build_specific_graph_model()
            self.ops['loss_eval'], self.ops['node_vec_eval'], self.ops['node_cel_eval'], self.ops['node_his_eval'], \
                            self.ops['predict'], self.ops['neg_predict'], self.ops['test_sample'] = self.build_specific_eval_graph_model()
    ...
    def _create_placeholders(self):
            ...
            self.placeholders['events_time_ph'] = [tf.placeholder(tf.float64, shape=[None], name='event%i_time_ph' % event)
                                                        for event in range(self._eventnum_batch)]
...
###########test placeholder###########################
            ...
            self.placeholders['event_time_eval_ph'] = tf.placeholder(tf.float64, shape=[None], name='event_time_eval_ph')
...
######################################################
    ...
    def _create_variables(self):
        ...
        self.weights['aggregator_attention'] = aggregator(self._h_dim, keep=self.placeholders['keep_prob'], weight_decay=self._weight_decay)
    ...
    def build_specific_graph_model(self):
        ...
        for event_id in range(self._eventnum_batch):
            ...
            post_fusion_his = tf.multiple(his_states, 1-his_states_attention)
        ...
        return loss_mean, node_vec[event_id+1], node_cel[event_id+1], node_his[event_id+1], predict, neg_predict, samples_val
    ...
    def build_specific_eval_graph_model(self):
        ...
        return loss_mean, node_vec, node_cel, node_his, predict, neg_predict, samples_val
        ...
        with open(self.log_file, 'w') as log_out:
            for epoch in range(self.params['num_epochs']):
                ...
                while not epoch_flag:
                    ...
                    fetches = [self.ops['loss'], self.ops['node_vec'], self.ops['node_cel'], self.ops['node_his'], self.ops['train_sample'], \
                               self.ops['global_step'], self.ops['lr'], self.ops['train_op']]
                    cost, self.node_embedding_cur, self.node_cellstates_cur, self.node_his_event, train_sample, step, lr, _ =\
    ...
    def validation(self, log_out):
        ...
        while not epoch_flag:
            fetches = [self.ops['loss'], self.ops['node_vec'], self.ops['node_cel'], self.ops['node_his'],
                       self.ops['pred'], self.ops['neg_pred'], self.ops['train_sample']]
            ...
            cost, self.node_embedding_cur, self.node_cellstates_cur, self.node_his_event, pred, neg_pred, train_sample = self.sess.run(fetches, feed_dict=feed_dict_valid)
            print(cost)
            print(train_sample)
    ...
    def test(self, log_out):
        ...
        while not epoch_flag:
            fetches = [self.ops['loss_eval'], self.ops['node_vec_eval'], self.ops['node_cel_eval'], self.ops['node_his_eval'],\
                self.ops['predict'], self.ops['neg_predict'], self.ops['test_sample']]
            feed_dict_test, epoch_flag = self.get_feed_dict_eval(is_init)
            # feed_dict_test, epoch_flag = self.get_feed_dict_eval()
            cost, self.node_embedding_cur, self.node_cellstates_cur, self.node_his_event, predict, neg_predict, test_sample = \
