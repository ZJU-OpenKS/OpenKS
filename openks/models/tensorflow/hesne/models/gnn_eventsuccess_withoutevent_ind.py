...
class GnnEventModel_withoutevent_ind(BasicModel):
    ...
    def get_log_file(self):
        log_file = 'h_' + str(self.params['hidden_size']) + '_b_' + str(self.params['batch_event_numbers']) \
                    ...
                    + str(self.params['decay_step']) + '_k_' + str(self.params['keep_prob']) + 'without_event_neg5.log'
        return log_file

    def get_checkpoint_dir(self):
        checkpoint_dir = 'h_' + str(self.params['hidden_size']) + '_b_' + str(self.params['batch_event_numbers']) \
                    + '_l_' + str(self.params['learning_rate']) + '_d_' + str(self.params['decay']) + '_ds_' \
                    + str(self.params['decay_step']) + '_k_' + str(self.params['keep_prob']) + 'without_event_neg5'
...
######################################################
    ...
    def build_specific_eval_graph_model(self):
        ...
        neg_his_events_states = tf.nn.embedding_lookup(self._embedding_init, self.placeholders['negevent_nodes_history_eval_ph'])
        ...
        with open(self.log_file, 'w') as log_out:
            for epoch in range(self.params['num_epochs']):
                ...
                while not epoch_flag:
                    ...
                    #            self.ops['global_step']]
                    # cost, self.node_embedding_cur, self.node_cellstates_cur, step =\
                    ...
                    if step == 1 or step % (self.params['decay_step']/10) == 0:
                        ...
                        # log_out.write(
                        #     'Epoch {}\tStep {}\tcost:{:.6f}\tavgc:{:.6f}\n'.format(epoch, step, cost,
                        #                                                                        avgc))
                        # print('Epoch {}\tStep {}\tcost:{:.6f}\tavgc:{:.6f}'.format(epoch, step, cost,
                        #                                                                        avgc))
                        sys.stdout.flush()
                        log_out.flush()
                print('start valid')
