...
class GnnEventModel(BasicModel):
    ...
    def get_log_file(self, params):
        log_file = 'h_' + str(self.params['hidden_size']) + '_b_' + str(self.params['batch_event_numbers']) \
                    ...
                    + str(self.params['decay_step']) + '_k_' + str(self.params['keep_prob']) + 'dy'
        return log_file
    ...
    def make_model(self):
        ...
        with tf.variable_scope('graph_model'):
            ...
            if self.params['graph_rnn_cell'].lower() == 'lstm':
                self.ops['loss'], self.ops['node_vec'], self.ops['node_cel'], self.ops['last_time'] = self.build_specific_graph_model()
                self.ops['concat_embedding'] = self.get_concat_embedding()
                self.ops['predict'], self.ops['neg_predict'], self.ops['node_vec_eval'], self.ops['node_cel_eval'], self.ops['last_time_eval'] =\
                                                                        self.build_specific_eval_graph_model()
            else:
                self.ops['loss'], self.ops['node_vec'] = self.build_specific_graph_model()
                self.ops['predict'], self.ops['neg_predict'], self.ops['node_vec_eval'] = \
                                                                        self.build_specific_eval_graph_model()
...
######################################################
            ...
            self.placeholders['is_init'] = tf.placeholder(tf.bool, name='init_embedding')
    ...
    def _create_variables(self):
        ...
        self._embedding_init = [tf.get_variable('node_type%i_embedding_init' % (node_type), shape=[1, self._h_dim],
        ...
        if self.params['graph_rnn_cell'].lower() == 'lstm':
            self._cellstates_init = [tf.get_variable('node_type%i_cellstates_init'%(node_type),
                                          shape=[1, self._h_dim], dtype=tf.float64, trainable=False,
                                          initializer=tf.zeros_initializer()) for node_type in range(self._type_num)]

        # self._assign_ops = []
        # for node_type in range(self._type_num):
        #     self._assign_ops += [
        #     tf.assign(self._node_embedding[node_type], self.placeholders['node_embedding_ph'][node_type])]
        #     if self.params['graph_rnn_cell'].lower() == 'lstm':
        #         self._assign_ops += [
        #         tf.assign(self._node_cellstates[node_type], self.placeholders['node_cellstates_ph'][node_type])]
    ...
    def build_specific_graph_model(self):
        def init_embedding():
            self._node_embedding_init = [None for _ in range(self._type_num)]
            for node_type in range(self._type_num):
                self._node_embedding_init[node_type] = tf.tile(self._embedding_init[node_type], [self._num_node_type[node_type], 1])
            return self._node_embedding_init

        def init_cellstates():
            self._node_cellstates_init = [None for _ in range(self._type_num)]
            for node_type in range(self._type_num):
                self._node_cellstates_init[node_type] = tf.tile(self._cellstates_init[node_type], [self._num_node_type[node_type], 1])
        ...
        # node_vec[0] = self._node_embedding_st
        if self.params['graph_rnn_cell'].lower() == 'lstm':
        ...
        # reg_loss = 0.
        # for var in self.dense_layer.vars.values():
        #     reg_loss += self.weght_decay * tf.nn.l2_loss(var)
        ...
        if self.params['graph_rnn_cell'].lower() == 'lstm':
            return loss_mean, node_vec[event_id+1], node_cel[event_id+1], node_last_time[event_id+1]
        return loss_mean, node_vec[event_id+1], node_last_time[event_id+1]

    def get_concat_embedding(self):
        node_embedding = [None for _ in range(self._type_num)]
        for node_type in range(self._type_num):
            concat_states = tf.concat([self._node_embedding_st[node_type], self.placeholders['node_embedding_ph'][node_type]],1)
            node_embedding[node_type] = self.dense_layer(concat_states)
        return node_embedding

    def build_specific_eval_graph_model(self):
        node_vec = [None] * self._type_num
        node_last_time = [None] * self._type_num
        if self.params['graph_rnn_cell'].lower() == 'lstm':
        ...
        for node_type in range(self._type_num):
            ...
            last_time = tf.expand_dims(tf.nn.embedding_lookup(self.placeholders['nodes_type_last_time_ph'][node_type],
                                                              self.placeholders['events_nodes_type_eval_ph'][
                                                                  node_type]), 1)
            ...
            for node_type in range(self._type_num):
                ...
                # neg_states = self.dense_layer(neg_concate_states)
                #############embedding with gate#############
                neg_last_time = tf.expand_dims(
                    tf.nn.embedding_lookup(self.placeholders['nodes_type_last_time_ph'][node_type],
        ...
        # rmse = tf.reduce_mean(tf.square(tf.subtract(predict, tf.ones([self._eventnum_batch],dtype=tf.float64))))
        # negrmse = tf.reduce_mean(tf.square(tf.subtract(neg_predict, tf.zeros([self._eventnum_batch, self._neg_num],dtype=tf.float64))))
        return predict, neg_predict, node_vec, node_cel, node_last_time
    ...
    def train(self):
        ...
        self.node_embedding_cur = [np.zeros([self.params['n_nodes_pertype'][node_type], self.params['hidden_size']], dtype=float)
                                   for node_type in range(self.params['node_type_numbers'])]
        if self.params['graph_rnn_cell'].lower() == 'lstm':
            self.node_cellstates_cur = [np.zeros([self.params['n_nodes_pertype'][node_type], self.params['hidden_size']], dtype=float)
                                   for node_type in range(self.params['node_type_numbers'])]
        with open(self.log_file, 'w') as log_out:
            for epoch in range(self.params['num_epochs']):
                ...
                while not epoch_flag:
                    ...
                    is_init = False
                    if self.params['graph_rnn_cell'].lower() == 'lstm':
                        fetches = [self.ops['loss'], self.ops['node_vec'], self.ops['node_cel'], self.ops['last_time'],\
                                   self.ops['global_step'], self.ops['lr'], self.ops['train_op']]
                        cost, self.node_embedding_cur, self.node_cellstates_cur, self.last_time, step, lr, _ = \
                            self.sess.run(fetches, feed_dict=batch_feed_dict)
                    else:
                        fetches = [self.ops['loss'], self.ops['node_vec'], self.ops['last_time'], self.ops['global_step'],
                                   self.ops['lr'], self.ops['train_op']]
                        cost, self.node_embedding_cur, self.last_time, step, lr, _ = \
                            self.sess.run(fetches, feed_dict=batch_feed_dict)
                ...
                if valid_loss < best_loss:
                    ...
                    # save_trained_embeddings(self.node_embedding_st, self.params['embedding_out_st'])
                    # save_trained_embeddings(self.node_embedding_validcur, self.params['embedding_out_dy'])
                    # save_trained_embeddings(self.concat_embedding, self.params['embedding_out_concat'])
                    # print('save static embeddings to:' + self.params['embedding_out_st'])
                    # print('save dynamic embeddings to:' + self.params['embedding_out_dy'])
                    # if self.params['graph_rnn_cell'].lower() == 'lstm':
                    #     save_trained_embeddings(self.node_cellstates_validcur, self.params['cellstates_out_dy'])
                    #     print('save dynamic cellstates to:' + self.params['cellstates_out_dy'])
                    # print('save concat embeddings to:' + self.params['embedding_out_concat'])
                    sys.stdout.flush()
    ...
    def validation(self):
        ...
        epoch_flag = False
        is_init = False
        self.node_embedding_validcur = self.node_embedding_cur
        if self.params['graph_rnn_cell'].lower() == 'lstm':
            self.node_cellstates_validcur = self.node_cellstates_cur
        print('valid batches' + str(valid_batches_num))
        while not epoch_flag:
            batch_feed_dict, epoch_flag = self.get_batch_feed_dict('valid', is_init)
            if self.params['graph_rnn_cell'].lower() == 'lstm':
                fetches = [self.ops['loss'], self.ops['node_vec'], self.ops['node_cel'], self.ops['last_time']]
                cost, self.node_embedding_validcur, self.node_cellstates_validcur, self.last_time = self.sess.run(fetches, feed_dict=batch_feed_dict)
            else:
                fetches = [self.ops['loss'], self.ops['node_vec'], self.ops['last_time']]
                self.sess.run(self._assign_ops, feed_dict=batch_feed_dict)
                cost, self.node_embedding_validcur, self.last_time = self.sess.run(fetches, feed_dict=batch_feed_dict)
            if np.isnan(cost):
                print('Evaluation loss Nan!')
                sys.exit(1)
            valid_loss.append(cost)
        for node_type in range(self._type_num):
            batch_feed_dict[self.placeholders['node_embedding_ph'][node_type]] = self.node_embedding_validcur[node_type]
        fetches = self.ops['concat_embedding']
        self.concat_embedding = self.sess.run(fetches, feed_dict=batch_feed_dict)
    ...
    def test(self):
        ...
        labels = []
        test_data_neg_list = []
        self.node_embedding_testcur = self.node_embedding_validcur
        if self.params['graph_rnn_cell'].lower() == 'lstm':
            self.node_cellstates_testcur = self.node_cellstates_validcur
        # self.node_embedding_testcur = load_trained_embeddings(self.params['embedding_out_dy'])
        # self.node_cellstates_testcur = load_trained_embeddings(self.params['cellstates_out_dy'])
        while not epoch_flag:
            if self.params['graph_rnn_cell'].lower() == 'lstm':
                fetches = [self.ops['predict'], self.ops['neg_predict'], self.ops['node_vec_eval'], self.ops['node_cel_eval'], self.ops['last_time_eval']]
            else:
                fetches = [self.ops['predict'], self.ops['neg_predict'], self.ops['node_vec_eval'], self.ops['last_time_eval']]
            feed_dict_test, epoch_flag = self.get_feed_dict_test()
            test_data_neg = [None for _ in range(self._type_num)]
            for node_type in range(self._type_num):
                test_data_neg[node_type] = feed_dict_test[self.placeholders['negevents_nodes_type_eval_ph'][node_type]]
            test_data_neg_list.append(test_data_neg)
            if self.params['graph_rnn_cell'].lower() == 'lstm':
                predict, neg_predict, self.node_embedding_testcur, self.node_cellstates_testcur, self.last_time =\
                                                                        self.sess.run(fetches, feed_dict=feed_dict_test)
            else:
                predict, neg_predict, self.node_embedding_testcur, self.last_time = self.sess.run(fetches, feed_dict=feed_dict_test)
        ...
        with open('/home1/wyf/Projects/dynamic_network_embedding/data/yelp/processed_nv/test_neg.pkl', 'wb') as test_neg_root:
            pkl.dump(test_data_neg_list, test_neg_root)
    ...
    def get_batch_feed_dict(self, state, is_init):
        ...
        if state == 'train':
            ...
            for node_type in range(self._type_num):
                batch_feed_dict[self.placeholders['node_embedding_ph'][node_type]] = self.node_embedding_cur[node_type]
                if self.params['graph_rnn_cell'].lower() == 'lstm':
                    batch_feed_dict[self.placeholders['node_cellstates_ph'][node_type]] = self.node_cellstates_cur[node_type]
        elif state == 'valid':
            batch_data, epoch_flag = self.valid_data.next_batch()
            for node_type in range(self._type_num):
                batch_feed_dict[self.placeholders['node_embedding_ph'][node_type]] = self.node_embedding_validcur[node_type]
                if self.params['graph_rnn_cell'].lower() == 'lstm':
                    batch_feed_dict[self.placeholders['node_cellstates_ph'][node_type]] = self.node_cellstates_validcur[node_type]
        else:
            print('state wrong')
    ...
    def get_feed_dict_test(self):
        ...
        feed_dict_test[self.placeholders['events_time_eval_ph']] = test_data[-1]
        for node_type in range(self._type_num):
        ...
        for node_type in range(self._type_num):
            ...
            feed_dict_test[self.placeholders['nodes_type_last_time_ph'][node_type]] = self.last_time[node_type]
