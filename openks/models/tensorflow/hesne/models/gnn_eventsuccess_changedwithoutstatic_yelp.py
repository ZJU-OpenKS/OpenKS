...
class GnnEventModel_changed_ws(BasicModel):
    ...
    def make_model(self):
        ...
        with tf.variable_scope('graph_model'):
            ...
            self.ops['loss_eval'], self.ops['predict'], self.ops['neg_predict'], self.ops['node_vec'], self.ops['node_cel'] =\
...
######################################################
    ...
    def build_specific_graph_model(self):
        ...
        for event_id in range(self._eventnum_batch):
            ...
            for node_type in range(self._type_num):
                ...
                states_aggregated = tf.reduce_mean(states_dy, axis=0, keepdims=True)
            ...
            for neg in range(self._neg_num):
                for node_type in range(self._type_num):
                    ...
                    neg_states_aggregated = tf.reduce_mean(neg_states_dy, axis=0, keepdims=True)
    ...
    def validation(self):
        ...
        epoch_flag = False
        self.node_embedding_validcur = [None for _ in range(self._type_num)]
        self.node_cellstates_validcur = [None for _ in range(self._type_num)]
        for node_type in range(self._type_num):
            self.node_embedding_validcur[node_type] = self._node_embedding_init[node_type].eval(session=self.sess)
            if self.params['graph_rnn_cell'].lower() == 'lstm':
                self.node_cellstates_validcur[node_type] = self._node_cellstates_init[node_type].eval(session=self.sess)
        while not epoch_flag:
            fetches = [self.ops['loss_eval'], self.ops['node_vec'], self.ops['node_cel']]
            feed_dict_valid, epoch_flag = self.get_feed_dict_valid()
            cost, self.node_embedding_validcur, self.node_cellstates_validcur = self.sess.run(fetches, feed_dict=feed_dict_valid)
            valid_loss.append(cost)
        return np.mean(valid_loss)
    ...
    def test(self):
        ...
        self.node_cellstates_testcur = [None for _ in range(self._type_num)]
        for node_type in range(self._type_num):
            self.node_embedding_testcur[node_type] = self._node_embedding_init[node_type].eval(session=self.sess)
            if self.params['graph_rnn_cell'].lower() == 'lstm':
                self.node_cellstates_testcur[node_type] = self._node_cellstates_init[node_type].eval(session=self.sess)
        ...
        while not epoch_flag:
            ...
            predict, neg_predict, self.node_embedding_testcur, self.node_cellstates_testcur = \
