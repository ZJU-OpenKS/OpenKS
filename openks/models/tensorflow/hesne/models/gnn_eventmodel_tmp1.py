...
class GnnEventModel(BasicModel):
    ...
    def compute_final_node_representations_perbatch(self):
        ...
        batch_vec_part = [None for node_type in range(self._type_num)]
        batch_cel_part = [None for node_type in range(self._type_num)]
        node_vec = [None for node_type in range(self._type_num)]
        node_cel = [None for node_type in range(self._type_num)]
        for node_type in range(self._type_num):
            batch_vec_part[node_type] = tf.dynamic_partition(self._node_embedding[node_type], self.placeholders['batch_partition_idx_ph'][node_type], 2)
            batch_cel_part[node_type] = tf.dynamic_partition(self._node_cellstates[node_type], self.placeholders['batch_partition_idx_ph'][node_type], 2)
            node_vec[node_type] = batch_vec_part[node_type][1]
            node_cel[node_type] = batch_cel_part[node_type][1]
        ...
        for event_id in range(self._eventnum_batch):
            ...
            with tf.variable_scope('event_id%i'%(event_id)):
                for node_type in range(self._type_num):
                    nodes_states_event_type = tf.nn.embedding_lookup(node_vec[node_type], self.placeholders['events_nodes_type_ph'][event_id][node_type])
                    nodes_cellstates_event_type = tf.nn.embedding_lookup(node_cel[node_type], self.placeholders['events_nodes_type_ph'][event_id][node_type])
                ...
                for node_type in range(self._type_num):
                    ...
                    node_vec_part = tf.dynamic_partition(node_vec[node_type], self.placeholders['event_partition_idx_ph'][event_id][node_type], 2)
                    target_states_event_type = tf.nn.embedding_lookup(node_vec[node_type], self.placeholders['events_nodes_type_ph'][event_id][node_type])
                    target_cellstates_event_type = tf.nn.embedding_lookup(node_cel[node_type], self.placeholders['events_nodes_type_ph'][event_id][node_type])
                    ...
                    for neg_radio in range(self._negative_ratio):
                        ...
                        neg_target_states_event_type = tf.nn.embedding_lookup(node_vec[node_type], self.placeholders['events_negnodes_type_ph'][event_id][node_type][neg_radio])
                    ...
                    node_vec[node_type] = tf.dynamic_stitch(self.placeholders['event_stitch_idx_ph'][event_id][node_type], [node_vec_part[0], new_target_states_event_type])
                    node_cel[node_type] = tf.dynamic_stitch(self.placeholders['event_stitch_idx_ph'][event_id][node_type], [node_vec_part[0], new_target_cellstates_event_type])

        for node_type in range(self._type_num):
            self._node_embedding[node_type] = tf.dynamic_stitch(self.placeholders['batch_stitch_idx_ph'][node_type], [batch_vec_part[node_type][0], node_vec[node_type]])
            self._node_cellstates[node_type] = tf.dynamic_stitch(self.placeholders['batch_stitch_idx_ph'][node_type], [batch_cel_part[node_type][0], node_cel[node_type]])
