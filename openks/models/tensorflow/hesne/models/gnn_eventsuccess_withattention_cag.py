...
class GnnEventModel_withattention_rev(BasicModel):
    ...
    def make_model(self):
        with tf.variable_scope('graph_model'):
            ...
            self.ops['loss'], self.ops['node_vec'], self.ops['node_cel'], self.ops['node_his'], self.ops['pred'], \
                                                    self.ops['neg_pred'] = self.build_specific_graph_model()
            self.ops['loss_eval'], self.ops['node_vec_eval'], self.ops['node_cel_eval'], self.ops['node_his_eval'], \
    ...
    def _create_placeholders(self):
            ...
            self.placeholders['events_time_ph'] = [tf.placeholder(tf.float64, shape=[None],
            ...
            self.placeholders['nodes_history_time_ph'] = tf.placeholder(tf.int32,
...
###########test placeholder###########################
            ...
            self.placeholders['event_time_eval_ph'] = tf.placeholder(tf.float64, name='event_time_eval_ph')
...
######################################################
    ...
    def _create_variables(self):
        ...
        # self.weights['history_attention'] = History_attention(self._h_dim)
    ...
    def build_specific_graph_model(self):
        ...
        for event_id in range(self._eventnum_batch):
            ...
            his_states = tf.where(self.placeholders['has_neighbor'][event_id], his_states, dy_states)
            his_states_attention = self.weights['history_attention']((dy_states, his_states))
            pre_fusion_his = tf.multiply(his_states, his_states_attention)
            # post_fusion_his = tf.multiply(his_states, 1-his_states_attention)
            ...
            for neg in range(self._neg_num):
                ...
                neg_his_states_attention = self.weights['history_attention']((neg_dy_states, neg_his_states))
                neg_post_fusion_his = tf.multiply(neg_his_states, 1-neg_his_states_attention)
                ...
                # post_neg_dy_states = tf.multiply(neg_dy_states, 1+neg_post_fusion_his)
                # neg_event_states = post_neg_dy_states*tf.expand_dims(neg_event_weights, 1)
            ...
            pre_fusion_his_part = tf.dynamic_partition(pre_fusion_his, self.placeholders['events_nodes_type_ph'][event_id], self._type_num)
            # post_fusion_his_part = tf.dynamic_partition(post_fusion_his, self.placeholders['events_nodes_type_ph'][event_id], self._type_num)
            for node_type in range(self._type_num):
                ...
                send_states_pertype = tf.concat([send_states_pertype, pre_fusion_his_part[node_type]], 1)
        ...
        ######### change node his subevent
            ...
            # samples_subevent_time = tf.tile(self.placeholders['events_time_ph'][event_id], [tf.shape(node_history_time_subpart)[0]])
        ...
        return loss_mean, node_vec[event_id+1], node_cel[event_id+1], node_his[event_id+1], predict, neg_predict
    ...
    def build_specific_eval_graph_model(self):
        ...
        pre_fusion_his_part = tf.dynamic_partition(pre_fusion_his, self.placeholders['event_nodes_type_eval_ph'], self._type_num)
        # post_fusion_his_part = tf.dynamic_partition(post_fusion_his, self.placeholders['event_nodes_type_eval_ph'], self._type_num)
        for node_type in range(self._type_num):
            target_embedding = node_vec_part[node_type+1]
            target_celstates = node_cel_part[node_type+1]
            # target_deltatime = tf.expand_dims(node_detatime_part[node_type], 1)
        ...
        return loss_mean, node_vec, node_cel, node_his, predict, neg_predict
        ...
        with open(self.log_file, 'w') as log_out:
            for epoch in range(self.params['num_epochs']):
                ...
                while not epoch_flag:
                    ...
                    fetches = [self.ops['loss'], self.ops['node_vec'], self.ops['node_cel'], self.ops['node_his'],\
                               self.ops['global_step'], self.ops['lr'], self.ops['train_op']]
                    cost, self.node_embedding_cur, self.node_cellstates_cur, self.node_his_event, step, lr, _ =\
    ...
    def validation(self, log_out):
        ...
        while not epoch_flag:
            ...
            cost, self.node_embedding_cur, self.node_cellstates_cur, self.node_his_event, pred, neg_pred = self.sess.run(fetches, feed_dict=feed_dict_valid)
            print(cost)
            # print(train_sample)
    ...
    def test(self, log_out):
        ...
        is_init = False
        while not epoch_flag:
            fetches = [self.ops['loss_eval'], self.ops['node_vec_eval'], self.ops['node_cel_eval'], self.ops['node_his_eval'],\
            ...
            cost, self.node_embedding_cur, self.node_cellstates_cur, self.node_his_event, predict, neg_predict = \
