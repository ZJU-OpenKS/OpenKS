...
class GnnEventModel_withattention_new(BasicModel):
    ...
    def get_log_file(self):
        log_file = 'h_' + str(self.params['hidden_size']) + '_b_' + str(self.params['batch_event_numbers']) \
                    ...
                    + str(self.params['max_his_num']) + '_n_' + str(self.params['negative_ratio']) + '_ns_' + str(self.params['negative_sampling']) + '_new.log'
        return log_file
    ...
    def make_model(self):
        with tf.variable_scope('graph_model'):
            ...
            self.ops['loss'], self.ops['node_his'], self.ops['pred'], self.ops['neg_pred'] = self.build_specific_graph_model()
...
######################################################
    ...
    def _create_variables(self):
        ...
        self._cellstates_init = tf.get_variable('nodes_cellstates_init', shape=[self._num_node, self._h_dim],
                                                dtype=tf.float64, trainable=True,
                                                initializer=tf.contrib.layers.xavier_initializer(uniform=False, seed=cur_seed))
    ...
    def build_specific_graph_model(self):
        def init_embedding():
            self._node_embedding_init = self._embedding_init
            return self._node_embedding_init
        def init_cellstates():
            self._node_cellstates_init = self._cellstates_init
        ...
        # node_vec[0] = self._embedding_init
        ...
        # node_cel[0] = self._cellstates_init
        ...
        return loss_mean, node_his[event_id+1], predict, neg_predict

    def build_specific_eval_graph_model(self):
        def init_embedding():
            return self._embedding_init
        def init_cellstates():
            return self._cellstates_init
        def assign_embedding():
            return self.placeholders['nodes_embedding_ph']
        def assign_cellstates():
            return self.placeholders['nodes_cellstates_ph']
        node_vec = tf.cond(self.placeholders['is_init'], init_embedding, assign_embedding)
        node_cel = tf.cond(self.placeholders['is_init'], init_cellstates, assign_cellstates)
        # node_vec = self._embedding_init
        # node_cel = self._cellstates_init
        ...
        with open(self.log_file, 'w') as log_out:
            for epoch in range(self.params['num_epochs']):
                ...
                while not epoch_flag:
                    ...
                    fetches = [self.ops['loss'], self.ops['node_his'],\
                               self.ops['global_step'], self.ops['lr'], self.ops['train_op']]
                    cost, self.node_his_event, step, lr, _ =\
    ...
    def validation(self, log_out):
        ...
        while not epoch_flag:
            ...
            cost, self.node_his_event, pred, neg_pred = self.sess.run(fetches, feed_dict=feed_dict_valid)
            print(cost)
