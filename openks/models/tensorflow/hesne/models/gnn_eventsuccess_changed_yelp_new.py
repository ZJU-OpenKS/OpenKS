...
class GnnEventModel_changed_new(BasicModel):
    ...
    def _create_placeholders(self):
            ...
            for node_type in range(self._type_num+1)] for event in range(self._eventnum_batch)]
            ...
            # self.placeholders['cur_adjdelta_ph'] = [tf.sparse.placeholder(tf.float64, shape=[self._num_node, self._num_node])
            #                                 for event in range(self._eventnum_batch)]
            ...
            self.placeholders['neighbor_stitch_idx_ph'] = [[
                tf.placeholder(tf.int32, shape=[None]),
                tf.placeholder(tf.int32, shape=[None])
            ] for event in range(self._eventnum_batch)]
...
###########test placeholder###########################
            ...
            self.placeholders['event_stitch_idx_eval_ph'] = [tf.placeholder(tf.int32, shape=[None])
                                                            for node_type in range(self._type_num+1)]
            ...
            self.placeholders['neighbor_stitch_idx_eval_ph'] = [
                tf.placeholder(tf.int32, shape=[None]),
                tf.placeholder(tf.int32, shape=[None])
            ]
######################################################
    ...
    def _create_variables(self):
        ...
        # self._cur_adj = tf.get_variable('cur_adj', shape=[self._num_node, self._num_node],
    ...
    def build_specific_graph_model(self):
        ...
        # cur_adj = self.placeholders['cur_adj_ph']
        ...
        for event_id in range(self._eventnum_batch):
            ...
            # self._cur_adj = tf.sparse.add(self._cur_adj, self.placeholders['cur_adj_ph'][event_id])
            for node_type in range(self._type_num):
                states_dy = tf.nn.embedding_lookup(node_vec[event_id],
            ...
            for node_type in range(self._type_num):
                ...
                new_target_embedding.append(new_target_states_ch[1])
                    ...
                    # node_cel[event_id + 1] = tf.dynamic_stitch(
            ...
            # node_vec[event_id + 1] = tf.dynamic_stitch(
            #     self.placeholders['event_stitch_idx_ph'][event_id][node_type],
            #     [node_vec_part[0], new_target_embedding])
            node_cel[event_id+1] = tf.dynamic_stitch(
                self.placeholders['event_stitch_idx_ph'][event_id], new_target_celstates)
            node_vec[event_id+1] = tf.dynamic_stitch(
                self.placeholders['event_stitch_idx_ph'][event_id], new_target_embedding)

            #####states propagation#############
            # if self.params['graph_rnn_cell'].lower() == 'lstm':


            # event_nodes = tf.concat(self.placeholders['events_nodes_type_ph'][event_id], axis=0)
            ...
            # type_ds = tf.sparse.reduce_sum(self.placeholders['cur_adj_ph'][event_id], 0)
            # type_d = tf.diag(tf.reciprocal(type_ds))
            # new_target_celstates = tf.nn.embedding_lookup(node_cel[event_id+1], event_nodes)
            # new_target_celstates = tf.matmul(type_wt, new_target_celstates)
            # new_target_celstates = tf.matmul(type_d, new_target_celstates)
            # new_neighbor_celstates = tf.sparse_tensor_dense_matmul(self.placeholders['cur_adj_ph'][event_id], new_target_celstates)
            # node_cel_part = tf.dynamic_partition(node_cel[event_id+1],
            #                         self.placeholders['neighbor_partition_idx_ph'][event_id], 2)
            # node_vec_part = tf.dynamic_partition(node_vec[event_id+1],
            #                         self.placeholders['neighbor_partition_idx_ph'][event_id], 2)
            # new_neighbor_celstates = node_cel_part[1]+new_neighbor_celstates
            # new_neighbor_embedding = tf.tanh(new_neighbor_celstates)
            #
            # node_cel[event_id+1] = tf.dynamic_stitch(self.placeholders['neighbor_stitch_idx_ph'][event_id],
            #                             [node_cel_part[0], new_neighbor_celstates])
            # node_vec[event_id+1] = tf.dynamic_stitch(self.placeholders['neighbor_stitch_idx_ph'][event_id],
            #                             [node_vec_part[0], new_neighbor_embedding])


                # node_cel[event_id+1] = tf.add(node_cel[event_id+1],
                #     tf.sparse_tensor_dense_matmul(tf.sparse.transpose(self.placeholders['cur_adj_ph'][event_id]), new_target_celstates))
    ...
    def build_specific_eval_graph_model(self):
        ...
        # self._cur_adj = tf.sparse.add(self._cur_adj, self.placeholders['cur_adj_eval_ph'])
        for node_type in range(self._type_num):
            states_dy = tf.nn.embedding_lookup(self.placeholders['node_embedding_ph'],
        ...
        for node_type in range(self._type_num):
            ...
            # node_cel = tf.dynamic_stitch(self.placeholders['event_stitch_idx_eval_ph'][node_type],
            #                                                         [node_cel_part[0], new_target_celstates])
            # else:
            ...
            # node_vec = tf.dynamic_stitch(self.placeholders['event_stitch_idx_eval_ph'][node_type],
            #                              [node_vec_part[0], new_target_embedding])
            ...
            #     [node_last_time_part[0], new_last_time])
        node_vec = tf.dynamic_stitch(self.placeholders['event_stitch_idx_eval_ph'], new_target_embedding)
        node_cel = tf.dynamic_stitch(self.placeholders['event_stitch_idx_eval_ph'], new_target_celstates)

        #####states propagation#############
        # if self.params['graph_rnn_cell'].lower() == 'lstm':


        # event_nodes = tf.concat(self.placeholders['events_nodes_type_eval_ph'], axis=0)
        #
        # type_ws = [tf.tile([self.weights['type_weights_scalar'][node_type]],
        ...
        # type_w = tf.concat(type_ws, 0)
        # type_wt = tf.diag(type_w)
        # type_ds = tf.sparse.reduce_sum(self.placeholders['cur_adj_eval_ph'], 0)
        # type_d = tf.diag(tf.reciprocal(type_ds))
        # new_target_celstates = tf.nn.embedding_lookup(node_cel, event_nodes)
        # new_target_celstates = tf.matmul(type_wt, new_target_celstates)
        # new_target_celstates = tf.matmul(type_d, new_target_celstates)
        # new_neighbor_celstates = tf.sparse_tensor_dense_matmul(self.placeholders['cur_adj_eval_ph'], new_target_celstates)
        # node_cel_part = tf.dynamic_partition(node_cel,
        ...
        # new_neighbor_embedding = tf.tanh(new_neighbor_celstates)
        #
        # node_cel = tf.dynamic_stitch(self.placeholders['neighbor_stitch_idx_eval_ph'],
        #                                            [node_cel_part[0], new_neighbor_celstates])
        # node_vec = tf.dynamic_stitch(self.placeholders['neighbor_stitch_idx_eval_ph'],
        #                                            [node_vec_part[0], new_neighbor_embedding])


            # node_cel = tf.add(node_cel,
            #             tf.sparse_tensor_dense_matmul(tf.sparse.transpose(self.placeholders['cur_adj_eval_ph']),
            #                                     new_target_celstates))
        # node_vec = tf.tanh(node_cel)
        ####################################
    ...
    def get_batch_feed_dict(self):
        ...
        for event in range(self._eventnum_batch):
            ...
            neighbor_nodes = []
            # for neighbor in range(self._num_node):
            #     if sum(self.cur_adj[neighbor, node_list]) > 0:
            ...
            # edge_indicese = np.asarray(edge_indicese, dtype=np.int32)
            # edge_values = np.asarray(edge_values, dtype=np.int32)
            # edge_shape = np.array([self._num_node, self._num_node], dtype=np.int32)
            # batch_feed_dict[self.placeholders['cur_adj_ph'][event]] = self.cur_adj
            batch_feed_dict[self.placeholders['cur_neighbor_ph'][event]] = np.asarray(neighbor_nodes, dtype=np.int32)
            ...
            batch_feed_dict[self.placeholders['neighbor_stitch_idx_ph'][event][0]] = np.where(neighbor_partition==0)[0].tolist()
            batch_feed_dict[self.placeholders['neighbor_stitch_idx_ph'][event][1]] = np.where(neighbor_partition==1)[0].tolist()
            ...
            for node_type in range(self._type_num):
                ...
                batch_feed_dict[self.placeholders['event_stitch_idx_ph'][event][node_type+1]] = \
                    np.where(event_partition==(node_type+1))[0].tolist()
            ...
            batch_feed_dict[self.placeholders['event_stitch_idx_ph'][event][0]] = \
    ...
    def get_feed_dict_eval(self, type):
        ...
        # edge_values = np.asarray(edge_values, dtype=np.int32)
        # edge_shape = np.array([self._num_node, self._num_node], dtype=np.int32)
        feed_dict_eval[self.placeholders['cur_neighbor_eval_ph']] = np.asarray(neighbor_nodes, dtype=np.int32)
        ...
        feed_dict_eval[self.placeholders['neighbor_stitch_idx_eval_ph'][0]] = np.where(neighbor_partition == 0)[0].tolist()
        feed_dict_eval[self.placeholders['neighbor_stitch_idx_eval_ph'][1]] = np.where(neighbor_partition == 1)[0].tolist()
        # feed_dict_eval[self.placeholders['cur_adj_eval_ph']] = self.cur_adj

        feed_dict_eval[self.placeholders['node_embedding_ph']] = self.node_embedding_evalcur
        # if self.params['graph_rnn_cell'].lower() == 'lstm':
        feed_dict_eval[self.placeholders['node_cellstates_ph']] = self.node_cellstates_evalcur
