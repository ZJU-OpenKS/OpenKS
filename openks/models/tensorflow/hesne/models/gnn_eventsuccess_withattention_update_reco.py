...
class GnnEventModel_withattention_update_reco(BasicModel):
    ...
    def get_log_file(self):
        log_file = 'h_' + str(self.params['hidden_size']) + '_b_' + str(self.params['batch_event_numbers']) \
                   ...
                   + str(self.params['max_his_num']) + '_n_' + str(self.params['negative_ratio']) + '_ns_' + str(self.params['negative_sampling']) + '_witht_update_reco.log'
        return log_file
    ...
    def get_checkpoint_dir(self):
        checkpoint_dir = 'h_' + str(self.params['hidden_size']) + '_b_' + str(self.params['batch_event_numbers']) \
                         + '_l_' + str(self.params['learning_rate']) + '_d_' + str(self.params['decay']) + '_ds_' \
                         + str(self.params['decay_step']) + '_k_' + str(self.params['keep_prob']) + '_' + str(self.params['his_type']) + '_' \
                         + str(self.params['max_his_num']) + '_n_' + str(self.params['negative_ratio']) + '_ns_' + str(self.params['negative_sampling']) + '_witht_update_reco'
        return checkpoint_dir

    def make_model(self):
        with tf.variable_scope('graph_model'):
            self.prepare_specific_graph_model()
            self.ops['predict'] = self.build_specific_evo_graph_model()
...
######################################################
    ...
    def build_specific_evo_graph_model(self):
        ...
        his_events_states = tf.nn.embedding_lookup(node_vec,
    ...
    def build_specific_eval_graph_model(self):
        ...
        neg_his_events_states = tf.nn.embedding_lookup(node_vec,
    ...
    def test(self):
        ...
        test_idx = 0
        while not epoch_flag:
            feed_dict_evo, eval_data, epoch_flag = self.get_feed_dict_evo()
            fetches_evo = [self.ops['predict']]
            if test_idx not in test_ids:
                predict = self.sess.run(fetches_evo, feed_dict=feed_dict_evo)
            else:
                neg_predict_score = []
                neg_flag = False
                ...
                while not neg_flag:
                    ...
                    feed_dict_eval, start_idx, neg_flag = self.get_feed_dict_eval(eval_data_neg_list, start_idx)
                    neg_predict = self.sess.run(fetches_eval, feed_dict=feed_dict_eval)
                    neg_predict_score.extend(neg_predict)
                predict = self.sess.run(fetches_evo, feed_dict=feed_dict_evo)
                val_preds.append(predict[0])
    ...
    def get_feed_dict_eval(self, eval_data_neg_list, start_idx):
        feed_dict_eval = {}
        sub_events={**self._sub_events_train, **self._sub_events_valid, **self._sub_events_test}
        events_time = {**self._events_time_train, **self._events_time_valid, **self._events_time_test}
